...
describe('static file serving', () => {
  ...
  beforeEach(async () => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'casper-static-'));
    fs.mkdirSync(path.join(root, 'assets'));
    fs.writeFileSync(path.join(root, 'index.html'), '<html>shell</html>');
    fs.writeFileSync(path.join(root, 'assets', 'at-startup.js'), 'a');
    app = Fastify();
    const fastifyStatic = (await import('@fastify/static')).default;
    await app.register(fastifyStatic, { root });
    app.setNotFoundHandler((req, reply) => {
      const p = req.url.split('?')[0] ?? '';
      const looksLikeAsset = p.startsWith('/assets/') || /\.[a-zA-Z0-9]+$/.test(p);
      if (p.startsWith('/api/') || p.startsWith('/ws') || looksLikeAsset) {
  ...
  it('serves a file that appeared after registration', async () => {
    // The regression: with wildcard:false the route table is a snapshot taken at
    // registration, so a rebuild's new hashed assets 404 until restart even though
    // index.html is served fresh and points straight at them.
    fs.writeFileSync(path.join(root, 'assets', 'after-start.js'), 'b');
    const res = await app.inject({ method: 'GET', url: '/assets/after-start.js' });
    assert.equal(res.statusCode, 200);
  });

  it('still serves files present at registration', async () => {
    const res = await app.inject({ method: 'GET', url: '/assets/at-startup.js' });
    assert.equal(res.statusCode, 200);
  });

  it('falls back to index.html for a client-side route', async () => {
    const res = await app.inject({ method: 'GET', url: '/session/abc' });
    assert.equal(res.statusCode, 200);
    assert.match(res.body, /shell/);
  });

  it('404s a missing asset instead of returning HTML with the wrong MIME type', async () => {
    const res = await app.inject({ method: 'GET', url: '/assets/gone.js' });
    assert.equal(res.statusCode, 404);
    assert.doesNotMatch(res.body, /<html>/);
  });

  it('404s unknown api routes rather than serving the shell', async () => {
