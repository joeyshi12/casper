...
export function upsertChat(
  ...
  const existing = chats.find((s) => s.chatId === summary.chatId);
  ...
  const row =
    existing && existing.updatedAt.localeCompare(summary.updatedAt) > 0
      ...
      : summary;
  return [...chats.filter((s) => s.chatId !== summary.chatId), row].sort(byRecent);
}
 ...
 */
export function bumpChatToTop(
  chats: ChatSummary[],
  chatId: string,
  updatedAt: string,
): ChatSummary[] {
  const row = chats.find((s) => s.chatId === chatId);
  return row ? upsertChat(chats, { ...row, updatedAt }) : chats;
}
